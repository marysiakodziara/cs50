...
def lunhs_algorithm(x):
    ...
    prod_digits = 0
    while x > 0:
        every_other = int((2 * (int(x / 10) - (int(x / 100) * 10))))
        remaining_numb = int((x - (int(x / 10) * 10)))
        if every_other > 9:
            prod_digits = int(int(every_other / 10) + (every_other - (int(every_other / 10) * 10)))
        else :
